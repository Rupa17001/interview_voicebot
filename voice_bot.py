...
print(deep_api)
def record_audio(duration=7, samplerate=44100):
    st.write("Recording...")
    audio_data = sd.rec(int(samplerate * duration), samplerate=samplerate, channels=1, dtype=np.int16)
    sd.wait()
    filename = f"input/audio_{uuid.uuid4().hex}.wav"
...
def using_mistral(query_text):
    prompt = f"""[INST] You are an assitant named Rupa and you are sitting in an interview. You have to answewr the question from given data only : Name : Rupa 
